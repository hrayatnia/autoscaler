...
func TestLabelMatches(t *testing.T) {
	cases := []struct {
		...
		{"any-of: matches second", []string{"self-hosted", "subzero-infra"}, []string{"mac-docker-infra", "subzero-infra"}, true},
		{"any-of: matches none", []string{"self-hosted", "ubuntu-latest"}, []string{"mac-docker-infra", "subzero-infra"}, false},
		{"empty job labels", []string{}, []string{"mac-docker-backend"}, false},
		{"empty want", []string{"self-hosted", "subzero-infra"}, []string{}, false},
	}
